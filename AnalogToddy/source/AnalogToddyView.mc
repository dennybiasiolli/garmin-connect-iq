...
class AnalogToddyView extends Ui.WatchFace {
	var showSecond = true;
    ...
    function onLayout(dc) {
		...
		batt_x = (width_screen / 8 * 3) - (batt_width_rect/2) - (batt_width_rect_small/2);
		batt_y = (height_screen / 4) - (batt_height_rect/2);
    ...
    function onUpdate(dc) {
        ...
        // Draw the logo
    	var image = Ui.loadResource(Rez.Drawables.BackLogo);
    	//dc.drawBitmap(20,65,image);
    	dc.drawBitmap(20,90,image);
	...
	function drawDate(dc, text_color)
	{
        ...
        dc.setColor(text_color, Gfx.COLOR_TRANSPARENT);
        dc.drawText(width_screen / 8 * 6, (height_screen / 4), Gfx.FONT_TINY, dateStr, Gfx.TEXT_JUSTIFY_VCENTER);
	}
}