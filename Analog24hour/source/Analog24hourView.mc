...
class Analog24hourView extends Ui.WatchFace {
	var showSecond = true;
    ...
    function onLayout(dc) {
		...
		for(var i = 0; i < 24; i+=1)
		{
			...
			{
				hashMarksArray[i][0] = (i / 24.0) * Math.PI * 2;
	...
	function drawHands(dc, clock_hour, clock_min, clock_sec, hour_color, min_color, sec_color)
	{
        ...
        // compute the angle.
        hour = ( ( ( clock_hour % 24 ) * 60 ) + clock_min );
        hour = hour / (24 * 60.0);
        hour = hour * Math.PI * 2;
    ...
    function drawHashMarks(dc)
    {
        // Draw the numbers
        dc.setColor(Gfx.COLOR_DK_GRAY, Gfx.COLOR_TRANSPARENT);
        dc.drawText((width_screen/2), 0, Gfx.FONT_MEDIUM, "0",Gfx.TEXT_JUSTIFY_CENTER);
        ...
        dc.drawText(0, height_screen/2, Gfx.FONT_MEDIUM, " 18",Gfx.TEXT_JUSTIFY_LEFT + Gfx.TEXT_JUSTIFY_VCENTER);
